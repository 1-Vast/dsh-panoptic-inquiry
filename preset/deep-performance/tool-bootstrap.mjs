...
/** Bypass the minimal phase for delegated workers and routed deep work. */
export function shouldFastTrack(session, sections = []) {
  if ((session?.header?.delegationDepth ?? 0) > 0) return true
  return sections.some(section => DEEP_ROUTE_SECTIONS.has(section?.name))
}
...
function refresh(agent, policy, sections = []) {
  const session = agent?.session
  if (session === undefined) return undefined
  const state = stateFor(session)
  agentBySession.set(session, agent)
  if (!state.promoted && shouldFastTrack(session, sections)) state.promoted = true
  if (!state.promoted) {
...
export function apply(ctx, config) {
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    const state = refresh(agent, policy, assembled.sections)
