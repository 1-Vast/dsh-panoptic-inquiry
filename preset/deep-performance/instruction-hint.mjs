...
export function apply(ctx, config) {
  ...
  /** Last hinted compaction boundary per live Session. */
  ...
  ctx.on('agent/pre-step', async ({ agent, signal }, next) => {
    ...
    try {
      ...
      if (session === undefined || hinted.get(session) === status.boundary) return decision
      hinted.set(session, status.boundary)
